...
start_idx = content.find("// Outer atmosphere glow")
end_idx = content.find("                        }\n                    }") + 48 # include some of the closing braces

# simpler: find first "LazyColumn(" and we replace until right before that
lazy_idx = content.find("// City List")
if lazy_idx == -1:
    lazy_idx = content.find("LazyColumn(")

# Let's find exactly the block to replace using split/join or regex
import sys
...
for i, line in enumerate(lines):
    if "// Outer atmosphere glow" in line and start_line == -1:
        start_line = i
    if "drawContext.canvas.nativeCanvas.drawText" in line:
        end_line = i + 2 # include closing brace of if and forEach
